...
public class ComboDetailDao extends BaseDao{
    ...
    public boolean deleteBatch(Set<ComboDetails> comboDetailsSet){
        Session session = null;
    ...
    public Set<ComboDetails> getSeatTypeByComboId(int comboId){

        Session session = null;
        List<ComboDetails> comboDetailsList = new ArrayList<>();
        try{
            session = this.sessionFactory.openSession();
            comboDetailsList = session.createQuery("select distinct comboDetails FROM ComboDetails comboDetails " +
                    " where comboId = :combo_id " +
                    " and comboProductType = 'TICKET' ")
                    .setParameter("combo_id", comboId)
                    .list();
        }catch (HibernateException hEx){
            // Insert to database exception log
        ...
        return new HashSet<>(comboDetailsList);
    }
